...
void Trigger_Settings_Activate (void)
{
	...
	//If master cycle time is shorter than laser warmup or charging time, correct master cycle time.
	if ((Laser_Warmup >= Total_Cycle_Time) || (Gun_Charging >= Total_Cycle_Time) ) {
		if (Laser_Warmup > Gun_Charging) {
				Total_Cycle_Time=1.0+Laser_Warmup;
		} else {
				Total_Cycle_Time=1.0+Gun_Charging;
		}
		//Save this value of the cycle time to the global cycle time variable
		Master_cycle_time=Total_Cycle_Time;
	...
	Get_PLIF_Laser_Offsets(&PLIF_Offset);																			//Since the PLIF and para TS lasers are the same Q850s, we can use the same warmup time without too much issue																												
	
	if (Get_TS_Laser_Offsets(&PerpTS_Offset, &ParaTS_Offset, &TSLaser_Warmup)) {
		Adjusted_Laser_Warmup = Laser_Warmup - (Astrella_Synch+Arc+Bias1+PerpTS-PerpTS_Offset);		//This uses the sum of the TS laser offsets since the offset of the laser not in use is set to zero.
	}else {
		Adjusted_Laser_Warmup = Laser_Warmup - (Astrella_Synch+Arc+Bias1+ParaTS-ParaTS_Offset);		//This uses the sum of the TS laser offsets since the offset of the laser not in use is set to zero.
	}
	...
	SRSDG1_C = Total_Cycle_Time-Gun_Charging;
	SRSDG1_D = Total_Cycle_Time-Gun_Charging+Gun_Charging_Pulse_Length;
