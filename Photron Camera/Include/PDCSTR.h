...
typedef struct {
	char m_nTemperatureSEN;     /*SEN Board*/
	char m_nTemperatureCPU;     /*CPU Board*/
	char m_nTemperatureFPGA;    /*CPU FPGA*/
}PDC_TEMPERATURE_DATA_TYPE6, * PPDC_TEMPERATURE_DATA_TYPE6;
...
/*	GRAW Buffer Information */
typedef struct {
	long			m_nWidth;
	long			m_nHeight;
	unsigned short	m_nSensorBit;
	unsigned short	m_nRealBitPerPixel;
	unsigned long	m_nEffectiveBits;
	long			m_nInterleave;
	unsigned long	m_nBayer;
} PDC_GRAW_IMAGE_BUFF_INFO, *PPDC_GRAW_IMAGE_BUFF_INFO;

typedef struct {
	unsigned long	m_nInteleave;
	unsigned long	m_nBitDepth;
	unsigned long	m_nEffectiveBits;
	unsigned long	m_nBayer;
} PDC_GRAW_OPTION_INFO, *PPDC_GRAW_OPTION_INFO;

/*	GRAW Information */
typedef struct {
	void*						m_pRawInfo;
	PDC_GRAW_IMAGE_BUFF_INFO	m_ImageInfo;
	unsigned long				m_nFrameCount;
} PDC_GRAW, *PPDC_GRAW;


typedef void* PDC_GPU_CONVERTER;
typedef PDC_GPU_CONVERTER *PPDC_GPU_CONVERTER;

typedef struct
{
	unsigned long srcFbo;
	unsigned long srcFboTexture;
	unsigned long dstFbo;
	unsigned long dstFboTexture;
	unsigned long width;
	unsigned long height;
	unsigned long bitDepth;
} PDC_GPU_FBO_INFO, *PPDC_GPU_FBO_INFO;

/* Network Information */
typedef struct {
	unsigned long nIPAddress;
	unsigned long nSubnetmask;
	unsigned long nDefaultGateway;
	unsigned char macAddress[6];
	unsigned long nDeviceCode;
}PDC_NETWORK_INFO, * PPDC_NETWORK_INFO;
