...
ViSession 	DoubleProbeLeCroy_handle1;
ViSession 	DoubleProbeLeCroy_handle2;


//Set up the LeCroy scope
void DoubleProbeActivate(void)
{
	int 		result; 					//	Error-checking variable
	float		Double_Probe_VoltageRange=10;
	char		DoubleProbe_string[256]="Null";
	char		IP_Address_string1[64];
	char		IP_Address_string2[64];	
	...
	//Get LeCroy ip addresses from interface panel and open connection to the scope
	GetCtrlVal (DoubleProbe_panel, DoublePrbe_DBPRB_LeCroyAddress1, IP_Address_string1);
	GetCtrlVal (DoubleProbe_panel, DoublePrbe_DBPRB_LeCroyAddress2, IP_Address_string2);
	...
	result=	viOpen (Global_Visa_Session_Handle, IP_Address_string1, VI_NULL, VI_NULL, &DoubleProbeLeCroy_handle1);
	...
	result=	viOpen (Global_Visa_Session_Handle, IP_Address_string2, VI_NULL, VI_NULL, &DoubleProbeLeCroy_handle2);
...
void Write_DoubleProbeData(void)
{
	...
	Lecroy(DoubleProbeLeCroy_handle2, Double_Probe_RecordLength, &DoubleProbe_timestep, DoubleProbe_Array5, DoubleProbe_Array6, DoubleProbe_Array7, DoubleProbe_Array8); 
							  ...
							  void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			GetCtrlVal (DoubleProbe_panel, DoublePrbe_R_Stop,&DoubleProbe_RStart);
			...
			GetCtrlVal (DoubleProbe_panel, DoublePrbe_R_Position,&DoubleProbe_RPosition);
			
			//Send move command to probe motor
			TitanMotorMove(DoubleProbe_Probe_IP_Address, (DoubleProbe_Offset-DoubleProbe_RPosition)*10.0, (DoubleProbe_Offset-DoubleProbe_RStart)*10.0, DoubleProbe_COUNTSPERMM);		//Convert distances to mm from cm
							  ...
							  void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			//Send move command to probe motor
			TitanMotorMove(DoubleProbe_Probe_IP_Address, (DoubleProbe_Offset-DoubleProbe_RPosition)*10.0, (DoubleProbe_Offset-DoubleProbe_RStop)*10.0, DoubleProbe_COUNTSPERMM);		//Convert distances to mm from cm
							  ...
							  void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			//Send move command to probe motor
			TitanMotorHome(DoubleProbe_Probe_IP_Address);		//Convert distances to mm from cm
...
float Advance_DoubleProbe_Probe(void)
{
	...
	if (Probe_Active_Move_Flag) {
		...
		if ( (DoublePrbe_RIncrement + Current_Position) > DoublePrbe_RStop) {
			...
			TitanMotorMove(DoublePrbe_Probe_IP_Address, (DoublePrbe_Offset-Probe_Move)*10.0, (DoublePrbe_Offset-Current_Position)*10.0, DoublePrbe_COUNTSPERMM);		//Convert distances to mm from cm
		}
