...
int			McP_Andor_H_bin;
int			McP_Andor_V_bin;
int 		McP_Andor_cameraIndex = 1;		//select second camera, assume it is the McPherson camera and not the Thomson Scattering camera
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			//read the calibration facotr, counts per mm for this spectrometer
			GetCtrlVal (McPherson209_panel, McP209_McP209_Countspernm,&Countspernm_209);
			
			//open COM port to device and flush buffers
		   	result = OpenComConfig (McPherson209_COM_Port, COM_string, 9600, 0, 8, 1, 64, 64);
			...
			if (result < 0) {
				...
				SetComTime (McPherson209_COM_Port, 0.2);	
				...
				if (Target_Wavelength > Current_Wavelength) {
					ComWrt (McPherson209_COM_Port, "+%d", counts_to_move);		// send move command
				} else {
					ComWrt (McPherson209_COM_Port, "-%d", counts_to_move);		// send move command
				}
