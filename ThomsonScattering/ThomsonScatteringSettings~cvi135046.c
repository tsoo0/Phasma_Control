	...
	if (TS_Para_Perp_flag) {   //Perp choice
		...
		if(!result) {		
			...
			if (!simmermode) {
				...
				sprintf (agilent_string, "TRIG:SOUR POS\r");   
				result = viWrite (Agilent_handle, agilent_string, (unsigned int)strlen(agilent_string), VI_NULL);
...
void OpenThomsonScattering_Settings (void)  
{
    ...
    if ( (error = DRV_TEMPERATURE_OFF) ) {
		...
		SetCtrlVal (ThomsonScattering_panel, ThomsonSct_Andor_Temp_Actual,TS_Andor_Temp);	//If temp call failed, set temperature in window to 1000 degrees
	} else {
		SetCtrlVal (ThomsonScattering_panel, ThomsonSct_Andor_Temp_Actual,TS_Andor_Temp);
    }
