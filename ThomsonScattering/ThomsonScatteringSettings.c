...
int			Andor_Gain;
int			Andor_Pre_Amp;
int 		TS_Andor_cameraIndex = 0;			//select first camera, assume it is the Thomson camera and not the McPherson camera
int 		TS_Andor_cameraHandle;   			//Handle for first camera
int			TS_Andor_cameraInitializeFlag=0;	//Tracks if the TS Andor camera has already been initialized so it is not done again
int 		iCoolerStatus=0;
...
void ThomsonScatteringLaserControl(int simmermode)
{
	...
	if (TS_Para_Perp_flag) {   //Perp choice
		...
		//Get Rigol Address String
	 	GetCtrlVal (ThomsonScattering_panel, ThomsonSct_TS_Perp_Rigol_Add,TS_Rigol_Add);
...
void ThomsonScatteringScopeandCamera(void)
{
	...
	if(!result) {
		...
		sprintf (ThomsonScattering_string, ":CHAN1:COUP DC\r");   
		result = viWrite (ThomsonScatteringRigol_handle, (ViConstBuf)ThomsonScattering_string, (unsigned int)strlen(ThomsonScattering_string), &count);

		sprintf (ThomsonScattering_string, ":CHAN2:COUP DC\r");   
		result = viWrite (ThomsonScatteringRigol_handle, (ViConstBuf)ThomsonScattering_string, (unsigned int)strlen(ThomsonScattering_string), &count);
...
int Get_TS_Laser_Offsets (float *Perp_offset,float *Para_offset, float *Laser_Warmup)
{
	float perp_delay;
	float para_delay;
	float para_warmup;
	...
	//Get parallel or perp choice state and warmup times
 	GetCtrlVal (ThomsonScattering_panel, ThomsonSct_OrientationSwitch,&TS_Para_Perp_flag);
	GetCtrlVal (ThomsonScattering_panel, ThomsonSct_Q1500_Warmup,&perp_warmup);
	GetCtrlVal (ThomsonScattering_panel, ThomsonSct_Q850_Warmup,&para_warmup);
	...
	para_delay = (Q850_InstrinsicDelay+Q850_Q_delay+Q850_Circuit_Delay)/1.0E6;
	perp_delay = (Q1500_InstrinsicDelay+Q1500_Q_delay+Q1500_Circuit_Delay)/1.0E6;

	
	//Send laser warmup times back to master trigger panel and make it that the offset time of the laser not in use is zero. Flag is equal to 1 when Perp laser in use
	if (TS_Para_Perp_flag) {
		*Laser_Warmup = perp_warmup;
		*Para_offset = 0.0;
		*Perp_offset = perp_delay;
	} else {
		*Laser_Warmup = para_warmup;
		*Para_offset = para_delay;
		*Perp_offset = 0.0;
	}	
...
int ThomsonScatteringArm(void)
{
	...
	if (result) { //Arm the Rigol Oscilloscope if it is there
...
//Use this routine to create the wavelength axis for the TS data
...
void Write_ThomsonScatteringData(void)
{
	...
	ThomsonScattering_Array1 = malloc (TS_RecordLength*8);  //8 times the memory to account for 8-bit numbers
	ThomsonScattering_Array2 = malloc (TS_RecordLength*8);  //8 times the memory to account for 8-bit numbers
	ThomsonScattering_Array3 = malloc (TS_RecordLength*8);  //8 times the memory to account for 8-bit numbers
	ThomsonScattering_Array4 = malloc (TS_RecordLength*8);  //8 times the memory to account for 8-bit numbers
	timebase = malloc (TS_RecordLength*8);				  	//8 times the memory to account for 8-bit numbers
	...
	fprintf(outfile,"Laser_Orientation,Type_of_Collection_Optics,Q1500_Frequency,Q1500_InstrinsicDelay,Q1500_Q_delay,Q1500_Circuit_Delay,Q1500_QSW_Source,Q1500_DDG_Delay,Q1500_Firing_Time,Q850_Frequency,Q850_InstrinsicDelay,Q850_Q_delay,Q850_Circuit_Delay,Q850_QSW_Source,Q850_DDG_Delay,Q850_Firing_Time,Andor_Gain,Andor_Pre_Amp,Andor_Exp_Time,Andor_DDGDelay,Andor_GateWidth,Andor_H_bin,Andor_V_bin,McP_Current_Wlength,McP_GrooveDen,McP_AngleDiff,McP_FocalLength\n" );
	...
	AndorImage = malloc ((int)((2*Andor_Horiz_pixels/Andor_H_bin)*(Andor_Vert_pixels/Andor_V_bin)));	//Build an array equal to the size of the total number of image pixels after the image has been binned into a
	...
	AndorImageDouble = malloc ((int)((8*Andor_Horiz_pixels/Andor_H_bin)*(Andor_Vert_pixels/Andor_V_bin))); //Increase the size to account for splitting the image into 2 parts and the fact that a double is 64 bits (or 8 words).
	...
	for (j=0;j<(int)(Andor_Vert_pixels/Andor_V_bin);j++)  {  
		for (i=0;i<(int)(Andor_Horiz_pixels/Andor_H_bin);i++)  {  
			AndorImageDouble[j*(int)(Andor_Horiz_pixels/Andor_H_bin)+i]=(double)AndorImage[j*(int)(Andor_Horiz_pixels/Andor_H_bin)+i];
		}
	}
	
	VerticalIndex = malloc ((int)(8*Andor_Vert_pixels/Andor_V_bin));		//Since 8 bit intergaer
	...
	for (j=0;j<(int)((Andor_Horiz_pixels/Andor_H_bin)*(Andor_Vert_pixels/Andor_V_bin)/2.0);j++) {
		fprintf(outfile,"%f, %f, %f\n",TSWavelength[j],AndorImageDouble[j],AndorImageDouble[j+(int)((Andor_Horiz_pixels/Andor_H_bin)*(Andor_Vert_pixels/Andor_V_bin)/2.0)]);
	}
...
void OpenThomsonScattering_Settings (void)  
{
	...
	GetAvailableCameras(&TStotalCameras);
	...
	if (TStotalCameras == 1) {
		TS_Andor_cameraHandle = 1;
	} else {
		TS_Andor_cameraHandle =0;
	}
	
	//Connect to TS camera and update panel with current camera temperature	
	error = GetCameraHandle(TS_Andor_cameraIndex, &TS_Andor_cameraHandle);   	//Get Handle for first camera
    if (error != DRV_SUCCESS) {
        printf("Only one camera in system so step to find multiple cameras failed %u\n", error);
    }
		
	if (TS_Andor_cameraHandle > 0) {
		error = SetCurrentCamera(TS_Andor_cameraHandle);						//Choose to make the TS camera the default camera if more than one camera
	}
    ...
    if (error != DRV_SUCCESS) {
      	printf("Camera initialization failed %u\n", error);
    } else {
		TS_Andor_cameraInitializeFlag=1;										//Set the flag that this camera has been initialized
	}
	Delay(0.25);																//Delay to let camera initialize
	
	error = IsCoolerOn(&iCoolerStatus);
   	if (iCoolerStatus) {
		//Move control switch to on position if temperature system active
		SetCtrlVal (ThomsonScattering_panel, ThomsonSct_Andor_Camera_Cooler,1);
		error = GetTemperature(&TS_Andor_Temp);												//Update camera temperature in window. Function call will fail if camera temp system not active
 		SetCtrlVal (ThomsonScattering_panel, ThomsonSct_Andor_Temp_Actual,TS_Andor_Temp);	//Report value of camera temperature to panel
	} else {
		//Move control switch to off position if temperature system not active
		SetCtrlVal (ThomsonScattering_panel, ThomsonSct_Andor_Camera_Cooler,0);
		SetCtrlVal (ThomsonScattering_panel, ThomsonSct_Andor_Temp_Actual,1000);
    }
...
int CVICALLBACK CloseSimmer_ThomsonScattering_Settings (int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
		...
		void *callbackData, int eventData1, int eventData2)
{
	int		result;
	int		count=0;
	int 	McPherson207_handle;
	int		counts_to_move;
	float 	Target_Wavelength;
	float 	Countspernm_207;
	char 	send_string[64];
 	char	TS_McP_COM_string[30];	
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			GetCtrlVal (panel, ThomsonSct_TS_McP_Center,&Target_Wavelength);
			
			//Get McPherson 209 controller COM port string from settings panel
 			GetCtrlVal (panel, ThomsonSct_TS_McP_COM,TS_McP_COM_string);
			
			//Read the current position of the spectrometer
			GetCtrlVal (panel, ThomsonSct_TS_McP_Current_Wlengt,&Current_Wavelength);
			
			//read the calibration factor, counts per mm for this spectrometer
			GetCtrlVal (panel, ThomsonSct_McP_Countspernm,&Countspernm_207);
	
			//Open VISA session to COM port
			result=viOpen (Global_Visa_Session_Handle, TS_McP_COM_string, VI_NULL, VI_NULL, &McPherson207_handle);
			...
			if (result < 0) {
				...
				counts_to_move=fabs((int)((Target_Wavelength-Current_Wavelength)*Countspernm_207));
				if (Target_Wavelength > Current_Wavelength) {
					sprintf (send_string, "+%d\r\n", counts_to_move);  					// write info to string to send to instrument
				} else {
					sprintf (send_string, "-%d\r\n", counts_to_move);  					// write info to string to send to instrument
				}
				result = viWrite (McPherson207_handle, (ViConstBuf)send_string, (ViUInt32)strlen(send_string), &count);
				
				//Update the stored current position of the spectrometer to be the new position (assume the move command was successful)
				SetCtrlVal (panel, ThomsonSct_TS_McP_Current_Wlengt,Target_Wavelength);				
			}
			//Close session to instrument
			result=viClose (McPherson207_handle);
								  ...
								  void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			GetCtrlVal (panel, ThomsonSct_TS_McP_Current_Wlengt,&Current_Wavelength);
			GetCtrlVal (panel, ThomsonSct_TS_McP_Center,&Current_Wavelength);
...
int CVICALLBACK TS_Andor_Temp_Control (int panel, int control, int event,
								  void *callbackData, int eventData1, int eventData2)
{
	int 	result;
	int		Andor_Temp;
	...
	switch (event)
	{
			case EVENT_COMMIT:
				...
				if (result) {
					GetCtrlVal (panel, ThomsonSct_Andor_Temp,&Andor_Temp);
					CoolerON();
					error = GetTemperature(&Actual_Temp);
					SetCtrlVal (panel, ThomsonSct_Andor_Temp_Actual,Actual_Temp);
					error = SetTemperature(Andor_Temp);
				} else {
					GetTemperature(&Actual_Temp);
					error = SetCtrlVal (panel, ThomsonSct_Andor_Temp_Actual,Actual_Temp);
					CoolerOFF();
					error = SetCoolerMode(0);
					ShutDown();
				}
