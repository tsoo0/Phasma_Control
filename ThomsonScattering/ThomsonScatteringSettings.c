...
void Write_ThomsonScatteringData(void)
{
	...
	Write_to_PHASMA_2D_MDS(AndorImageDouble, TSWavelength, VerticalIndex, Channel1NameString);

	//Store raw photodiode data on drive folder, and push data to MDSPlus for Channel 1
	Write_to_PHASMA_MDS(timebase,ThomsonScattering_Array1,Channel2NameString);
	
	//Write TS spectrum to end of TS file. The two halves are stored side by side along with the wavelegnth data
...
void OpenThomsonScattering_Settings (void)  
{
	...
	if (!TS_Andor_cameraInitializeFlag) {
		...
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			//ASssume McPherson moved to target wavelength and update current wavelength to reflect the change
			SetCtrlVal (ThomsonScattering_panel, ThomsonSct_TS_McP_Current_Wlengt,Target_Wavelength);
