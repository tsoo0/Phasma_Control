...
void PLIFLaserControl(int simmermode)
{
	...
	//Get Laser pulse frequency
 	GetCtrlVal (PulsedLIF_panel, PLIF_Q850_Frequency,&Q850_Frequency);
		
	//Get Laser intrinsic delay
 	GetCtrlVal (PulsedLIF_panel, PLIF_Q850_InstrinsicDelay,&Q850_InstrinsicDelay);
		
	//Get Laser Q switch delay
 	GetCtrlVal (PulsedLIF_panel, PLIF_Q850_Q_delay,&Q850_Q_delay);
		
	//Get Laser circuit delay
 	GetCtrlVal (PulsedLIF_panel, PLIF_Q850_Circuit_Delay,&Q850_Circuit_Delay);
		
	//Get Laser warm up time
 	GetCtrlVal (PulsedLIF_panel, PLIF_Q850_Warmup,&Q850_Warmup);
				
	//Get Laser trigger source flag
 	GetCtrlVal (PulsedLIF_panel, PLIF_Q850_QSW_Source,&Q850_QSW_Source);
	...
	GetCtrlVal (PulsedLIF_panel, PLIF_Q850_SimmerDelay,&Q850_Simmer_Delay);
	...
	if(!result) {
		...
		//Turn on channel 1
		sprintf (rigol_string, ":OUTP1 OFF\r");   
		...
		sprintf (rigol_string, ":SOUR1:BURS OFF\r");   
		result = viWrite (PLIF_Rigol_handle, rigol_string, (unsigned int)strlen(rigol_string), VI_NULL);
...
void Get_PLIF_Laser_Offsets (float *PLIF_offset)
{
	float time1;
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			if (!result) {
				...
				sprintf (Angstrom_String, "LASER:WAVELENGTH?\r");   																										
				result = viWrite (AngstromWavemeter_handle, Angstrom_String, (unsigned int)strlen(Angstrom_String), VI_NULL);
				result = viRead (AngstromWavemeter_handle, Angstrom_String, 64, count);
				sscanf(Angstrom_String, "%f", &wavelength);			
			}
