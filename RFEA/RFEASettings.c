							  ...
							  void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			GetCtrlVal (RFEA_panel, RFEA_R_Stop,&RFEA_RStart);
			...
			GetCtrlVal (RFEA_panel, RFEA_R_Position,&RFEA_RPosition);
			
			//Send move command to probe motor
			TitanMotorMove(RFEA_Probe_IP_Address, (RFEA_Offset-RFEA_RPosition)*10.0, (RFEA_Offset-RFEA_RStart)*10.0, RFEA_COUNTSPERMM);		//Convert distances to mm from cm
			
			//Set current position to Start
			SetCtrlVal (RFEA_panel, RFEA_R_Position,RFEA_RStart);
							  ...
							  void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			//Send move command to probe motor
			TitanMotorMove(RFEA_Probe_IP_Address, (RFEA_Offset-RFEA_RPosition)*10.0, (RFEA_Offset-RFEA_RStop)*10.0, RFEA_COUNTSPERMM);		//Convert distances to mm from cm
			
			//Set current position to Start
			SetCtrlVal (RFEA_panel, RFEA_R_Position,RFEA_RStop);
							  ...
							  void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			//Send move command to probe motor
			TitanMotorHome(RFEA_Probe_IP_Address);		//Convert distances to mm from cm
			
			//Set current position to Offset
			SetCtrlVal (RFEA_panel, RFEA_R_Position,RFEA_Offset);
...
float Advance_RFEA_Probe(void)
{
	...
	//Open settings panel and load saved values
	RFEA_panel = LoadPanel (0, "BdotSettings.uir", RFEA);
	RecallPanelState (RFEA_panel, "Master_Control_Storage_File", RFEA_setup_state);
	...
	if (Probe_Active_Move_Flag) {
		...
		if ( (RFEA_RIncrement + Current_Position) > RFEA_RStop) {
			...
			TitanMotorMove(RFEA_Probe_IP_Address, (RFEA_Offset-Probe_Move)*10.0, (RFEA_Offset-Current_Position)*10.0, RFEA_COUNTSPERMM);		//Convert distances to mm from cm
		}
