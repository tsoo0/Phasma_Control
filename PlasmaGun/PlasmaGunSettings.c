...
#define 	Gun1_Arc_Power 2000.0		//The default max power of the charging system
#define 	Gun2_Arc_Power 2000.0		//The default max power of the charging system


//Global Variables for settings panel
int			PlasmaGun_setup_state = 501;
int			PlasmaGun_panel;
int			Gun1_Bias_Port;
int			Gun2_Bias_Port;
...
void PlasmaGun_ActivateSystem (int Guns_Power_State)  
{
	...
	if (result >= 0) {
		...
		result = viWrite (Gun1_Bias_handle, (ViConstBuf)PlasmaGun_string, (unsigned int)strlen(PlasmaGun_string), &count);
	...
	if (result >= 0) {
		...
		//Set local address
		sprintf (PlasmaGun_string, "*ADR 20\r");   
		result = viWrite (Gun2_Bias_handle, (ViConstBuf)PlasmaGun_string, (unsigned int)strlen(PlasmaGun_string), &count);
	...
	//Gun 1 Arc
	//Get Gun1 IP address string from settings panel
 	GetCtrlVal (PlasmaGun_panel, PlasmaGun_Gun1_Arc_IP,Gun1_Arc_IP);
	
	//Open communication to power supply using Visa calls
	result=viOpen (Global_Visa_Session_Handle, Gun1_Arc_IP, VI_NULL, VI_NULL, &Gun1_Arc_handle);
	...
	if (result >= 0) {
		...
		if ((Guns_Power_State) && (Gun1_Power_State) ){
			...
			//Set the requested voltage level
			sprintf (PlasmaGun_string, ":SOUR:VOLT %f\r\n",Gun1_Arc_Volts); 	
			result = viWrite (Gun1_Arc_handle, (ViConstBuf)PlasmaGun_string, (unsigned int)strlen(PlasmaGun_string), &count);

			//Let instrument catch up
			Delay (0.1);
	
			//Set the current level
			sprintf (PlasmaGun_string, ":SOUR:CURR %f\r\n",Gun1_Arc_Amps); 	
			result = viWrite (Gun1_Arc_handle, (ViConstBuf)PlasmaGun_string, (unsigned int)strlen(PlasmaGun_string), &count);
			...
			//Set the power level
			sprintf (PlasmaGun_string, ":SOUR:POW %f\r\n",Gun1_Arc_Power); 	
			result = viWrite (Gun1_Arc_handle, (ViConstBuf)PlasmaGun_string, (unsigned int)strlen(PlasmaGun_string), &count);

			//Let instrument catch up
			Delay (0.1);
	
			sprintf (PlasmaGun_string, "OUTP 1\r\n");   
			result = viWrite (Gun1_Arc_handle, (ViConstBuf)PlasmaGun_string, (unsigned int)strlen(PlasmaGun_string), &count);
		} else {
			//Set the requested voltage level
			sprintf (PlasmaGun_string, ":SOUR:VOLT 0.0\r\n"); 	
			result = viWrite (Gun1_Arc_handle, (ViConstBuf)PlasmaGun_string, (unsigned int)strlen(PlasmaGun_string), &count);

			//Let instrument catch up
			Delay (0.1);

			sprintf (PlasmaGun_string, "OUTP 0\r\n");   
			result = viWrite (Gun1_Arc_handle, (ViConstBuf)PlasmaGun_string, (unsigned int)strlen(PlasmaGun_string), &count);
		}

		//Close IP port
		result = viClose(Gun1_Arc_handle);
	}
	
	//Gun 2 Arc
	//Get Gun2 IP address string from settings panel
 	GetCtrlVal (PlasmaGun_panel, PlasmaGun_Gun2_Arc_IP,Gun2_Arc_IP);
	
	//Open communication to power supply using Visa calls
	result=viOpen (Global_Visa_Session_Handle, Gun2_Arc_IP, VI_NULL, VI_NULL, &Gun2_Arc_handle);
	...
	if (result >= 0) {
		...
		if ((Guns_Power_State) && (Gun2_Power_State) ){
			...
			//Set the requested voltage level
			sprintf (PlasmaGun_string, ":SOUR:VOLT %f\r\n",Gun2_Arc_Volts); 	
			result = viWrite (Gun2_Arc_handle, (ViConstBuf)PlasmaGun_string, (unsigned int)strlen(PlasmaGun_string), &count);
...
float ReadGunPressure(void)
{
	...
	result = viWrite (Regulator_handle, (ViConstBuf)Regulator_string, (unsigned int)strlen(Regulator_string), &count);

	//Read regulator pressure
	result = viRead (Regulator_handle, (ViConstBuf)Regulator_string, 8 ,&count);
						   ...
						   void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			TitanMotorMove(Gun_1_IP_Address, (Gun_1_Offset-Gun_1_R)*10.0, (Gun_1_Offset-Gun_1_R)*10.0, Gun_1_COUNTSPERMM);		//Convert distances to mm from cm
			TitanMotorMove(Gun_2_IP_Address, (Gun_2_Offset-Gun_2_R)*10.0, (Gun_2_Offset-Gun_2_R)*10.0, Gun_2_COUNTSPERMM);		//Convert distances to mm from cm
							   ...
							   void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			//Send move command to probe motor
			TitanMotorHome(Gun_1_IP_Address);		//Convert distances to mm from cm
			//Send move command to probe motor
			TitanMotorHome(Gun_2_IP_Address);		//Convert distances to mm from cm
