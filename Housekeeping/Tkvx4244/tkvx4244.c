			...
			ViBoolean resetDevice, ViPSession instrumentHandle) 
{
	...
	retError = viOpenDefaultRM(&resMgr);
	if (retError < VI_SUCCESS)
	{
			...
			ViChar errorMessage[])		
{
	...
	cmd[(ViInt32)strlen(cmd)]='\0';
	retError = viWrite (instrumentHandle, (ViBuf)cmd, (ViInt32)strlen(cmd), (ViPUInt32)&retCnt);
	if (retError<VI_SUCCESS) return retError;
...
ViChar dataArray[])
{
	...
	retError = viWrite (instrumentHandle, (ViBuf)cmd, (ViInt32)strlen(cmd),(ViPUInt32)&retCnt);
	if (retError) return retError;
    tkvx4244_sleep((ViInt32)delaytime);
...
ViInt16 numberOfSamples,ViInt32 startingAddress,ViInt16 channel,ViInt16 response[])
{
	...
	for(ii=0;ii<retCnt/2;ii++)
		response[ii]= (ViInt16)(((rdd[2*ii])<<8) + rdd[(2*ii)+1]);
	/* Free rdd */				 
   free(rdd);
...
ViInt16 gp1,ViInt16 gp2,ViInt16 gp3,ViInt16 gp4,ViInt16 all)
{
    ...
    sprintf (cmd, "ABORT (%s)\n",gplst);
    /* Send the command string */
	retError = viWrite (instrumentHandle,(ViBuf)cmd,(ViInt32)(ViInt32)strlen(cmd),(ViPUInt32)&retCnt);
		if (retError) return retError;
  return VI_SUCCESS;
}
/***************************************************************************
...
ViStatus tkvx4244_setupArrays (void)
{
	...
	if (tkvx4244_gNumberFound == 0)		/* note that tkvx4244_gNumberFound is a global */
	{
		...
		if (retError >= VI_SUCCESS)
		{
			...
			//Removed since no longer have VXI devices on a VXI bus
