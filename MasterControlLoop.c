...
#define	TS_max_record_length 262144
 ...
 void ClearSystemStatus(void)
{
	...
	//Reset the individual data flag switches
	SetCtrlVal (Master_Control_Panel, MasterCont_MasterTrigger_Data, 0);
 ...
 void SingleStepAdvance(void)
{
	...
	float 	Gun1_Pos=0.0;
	float 	Gun2_Pos=0.0;
	float   PulsedLIF_wavelength=0.0;
	...
	if(PulsedLIFFlag)   	{
		PulsedLIF_wavelength=Advance_PLIF_Laser();
		SetCtrlVal (Master_Control_Panel, MasterCont_PLIF_Laser_Wavelength, Get_PLIF_Wavelength());
	}
	
	//Dump positions to a file
	sprintf(outfilename, RawDataPath);
	strcat(outfilename, ShotNumberString);
	strcat(outfilename, "_");
	strcat(outfilename, "ProbeandLIF_Positions.txt");
...
int CVICALLBACK QBS_Settings (int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
...
void	Repeat_Cycle (void)
{
	...
	//If needed, get laser warmup and charging times and save them to a global variable so they can be used everywhere
...
void  main(void)
{
	...
	//Setup timer to periodically check if any Python processes are running
	SetCtrlAttribute (Master_Control_Panel, MasterCont_PyTimer, ATTR_ENABLED, 1);	
...
int CVICALLBACK manual_disarm_all(int panel, int control, int event, void *callbackData,
