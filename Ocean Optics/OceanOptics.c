/*****************************************************************************************
* To operate this Ocean Optics code requires the Ocean Optics Software Development Kit   *
* If there is a neeed for Ocean Optics spectrometer data, then we need to purchase the   *
* SDK and install it on the lab computer                                                 *
...
int		OceanOptics_panel;
int		OceanOptics_setup_state = 200;		//unique identifier for settings of setup panel
...
void AcquireSpectrum(int argc, char* argv[]) {
...
//Acquire Spectrum
int CVICALLBACK OceanOpticsAcquire (int panel, int control, int event,
...
void OpenOceanOptics_Settings (void)
{
	...
	OceanOptics_panel = LoadPanel (0, "OceanOpticsSettings.uir", OceanOptic);
	RecallPanelState (OceanOptics_panel, "Master_Control_Storage_File", OceanOptics_setup_state);
	
	//Update MDS shot number
	shotnum=getMDSCurrentShot();
		
	//Display Panel
	DisplayPanel (OceanOptics_panel);
	
	//Update shot number in panel
	SetCtrlAttribute (OceanOptics_panel, OceanOptic_MDS_Shot, ATTR_CTRL_VAL, shotnum);
	
	// Start interacting with user
    RunUserInterface ();
}

int CVICALLBACK Close_OceanOptics_Settings (int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			SavePanelState (OceanOptics_panel, "Master_Control_Storage_File", OceanOptics_setup_state);
			DiscardPanel(panel);
			break;
	}
	return 0;
