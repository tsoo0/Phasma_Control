...
class Andor:
    def __init__(self):
        ...
        if platform.system() == "Windows":
            if platform.architecture()[0] == "32bit":
                ...
                self.dll = cdll.LoadLibrary("C:\\Program Files (x86)\\Andor SOLIS\\Andor SDK\\atmcd64d.dll")
        # for Linux
    ...
    def SetAcquisitionMode(self, mode=2):
        #1: Single scan
        #2: Accumulate
        #3: Kinetic scan
    ...
    def SetDDGGateTime(self, delay, width):
        error = self.dll.SetDDGGateTime(delay, width)
        self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)
        return ERROR_CODE[error]
    
    def SetDDGGateStep(self, step):
        error = self.dll.SetDDGGateStep(step)
        self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)
        return ERROR_CODE[error]
    
    def SetMCPGain(self, gain):
        error = self.dll.SetMCPGain(gain)
        self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)
        return ERROR_CODE[error]
    
    def SetGateMode(self,gateMode):
        error = self.dll.SetGateMode(gateMode)
        self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)
        return ERROR_CODE[error]
    ...
    def GetAcquiredData(self,imageArray):
        ...
        self.imageArray = np.reshape(self.imageArray, (self.height, self.width))
        #self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)
        return ERROR_CODE[error]
    ...
    def SaveAsTxt(self, path):
        ...
        for i in range(len(self.imageArray)):
            for j in range(len(self.imageArray[0])):
                file.write(str(self.imageArray[i][j]) + ",")
            
            file.write("\n")

        file.close()

    def SaveAsRaw(self, path, typ):
        error = self.dll.SaveAsRaw(path, typ)
        self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)
        return ERROR_CODE[error]
    ...
    def SetFastExtTrigger(self, gainMode):
        error = self.dll.SetFastExtTrigger(gainMode)
        self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)
        return ERROR_CODE[error]   
    ...
    def SetDDGIOC(self, index):
        error = self.dll.SetDDGIOC(index)
        self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)
        self.outamp = index
        return ERROR_CODE[error]
    
    def SetDDGIOCTrigger(self, index):
        error = self.dll.SetDDGIOCTrigger(index)
        self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)
        self.outamp = index
        return ERROR_CODE[error]
    
    def SetDDGIOCNumber(self, index):
        error = self.dll.SetDDGIOCNumber(index)
        self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)
        self.outamp = index
        return ERROR_CODE[error]
    ...
    def GetNumberVSAmplitudes(self):
        noVSSpeeds = c_int()
        error = self.dll.GetNumberVSAmplitudes(byref(noVSSpeeds))
        self.noVSSpeeds = noVSSpeeds.value
        self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)
        return error
    def GetVSSpeed(self):
        ...
        return len(self.VSSpeeds)

    def GetVSAmplitudes(self):
        VSSpeed = c_float()

        self.VSSpeeds = []

        for i in range(self.noVSSpeeds):
            self.dll.GetVSSAmplitudes(i,byref(VSSpeed))
            self.preVSpeeds.append(VSSpeed.value)
    ...
    def SetDDGInsertionDelay(self, state):
        error = self.dll.SetDDGInsertionDelay(state)
        self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)
        return ERROR_CODE[error]
